...
@section('content')
    <div class="container mt-4">
        <div class="card-header bg-info text-white fw-semibold">
            <i class="fas fa-bullseye me-2"></i> Add New Future
        </div>
        ...
        {{-- Add Form --}}
        <form id="addFutureForm" action="{{ route('future.store') }}" method="POST" enctype="multipart/form-data"
            class="card p-4 shadow mb-4">
            @csrf
            <h5 class="mb-3">Create New Future</h5>

            <input type="text" name="title" class="form-control mb-2" placeholder="Title" required>
            <input type="text" name="short_description" class="form-control mb-2" placeholder="Short Description"
                required>
            ...
            <button type="button" class="btn btn-sm btn-secondary mb-3" onclick="addCount()" style="width:200px">+ Add
                Count</button>
            ...
            <div class="card p-4 shadow mb-4">
                <form action="{{ route('future.update', $future->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="text" name="short_description" class="form-control mb-2"
                        value="{{ $future->short_description }}" required>
                    ...
                    <div class="mb-3">
                        @foreach ($future->counts ?? [] as $index => $item)
                            <div class="d-flex gap-2 mb-2">
                                <input type="number" name="counts[{{ $index }}][count]" class="form-control"
                                    value="{{ $item['count'] }}" required>
                                <input type="text" name="counts[{{ $index }}][title]" class="form-control"
                ...
                <form action="{{ route('future.destroy', $future->id) }}" method="POST"
