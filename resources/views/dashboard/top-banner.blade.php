...
<div class="container mt-4">
    <h4 class="mb-4">Top Banner Settings</h4>

    <div class="card shadow-sm p-4">
        <form action="{{ route('top-banner.update', 'banner_image') }}" method="POST" enctype="multipart/form-data" class="mb-3">
            @csrf
        ...
        @foreach (['banner_title' => 'Title', 'banner_subtitle' => 'Subtitle', 'banner_description' => 'Description'] as $field => $label)
            <form action="{{ route('top-banner.update', $field) }}" method="POST" class="mb-3">
                ...
                @else
                    <input type="text" name="value" class="form-control" value="{{ old('value', $data[$field] ?? '') }}">
                @endif
