            ...
            <div class="card-body">
                @foreach ($reasons as $reason)
                    <div class="card p-3 shadow-sm mb-3 border">
                        <form action="{{ route('why.reason.update', $reason->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-12 mb-2">
                                    <textarea name="long_description" class="form-control" rows="3" placeholder="Long Description">{{ $reason->long_description }}</textarea>
                                </div>
                                <div class="col-md-6 mb-2">
                                    @if($reason->icon)
                                        <img src="{{ asset('storage/' . $reason->icon) }}" height="50" width="50" class="rounded mb-2">
                                    @endif
                                    <input type="file" name="icon" class="form-control">
                                </div>
                            </div>
